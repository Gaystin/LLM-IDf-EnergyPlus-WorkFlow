...
def build_item_metrics(freq_df: pd.DataFrame, run_df: pd.DataFrame, item_col: str) -> pd.DataFrame:
    ...
    # 2) across cities: take mean of city summaries (equal city weight)
    city_item = merged.groupby(["city", item_col], as_index=False).agg(
        city_avg_frequency=("frequency", "mean"),
    ...
    metrics = grp.agg(
        total_frequency=("city_avg_frequency", "mean"),
        total_frequency_q25=("city_avg_frequency", lambda s: s.quantile(0.25)),
        total_frequency_q75=("city_avg_frequency", lambda s: s.quantile(0.75)),
        mean_frequency=("city_mean_frequency", "mean"),
        run_coverage=("city_run_coverage", "sum"),
        city_coverage=("city", "nunique"),
        weighted_contribution=("city_weighted_value", "mean"),
        mean_best_saving_when_present=("city_mean_best_saving", "mean"),
        std_best_saving_when_present=("city_mean_best_saving", "std"),
        efficiency_per_frequency=("city_efficiency_per_frequency", "mean"),
...
def plot_freq_saving_scatter(metrics: pd.DataFrame, item_col: str, out_file: Path) -> None:
    ...
    display_label = _display_item_label(item_col)
    ax.set_xlabel("City-Balanced Mean Frequency (%)")
...
def plot_quadrant(metrics: pd.DataFrame, item_col: str, out_file: Path) -> None:
    ...
    x_mid = df["total_frequency"].mean()
    y_mid = df["efficiency_per_frequency"].mean()
