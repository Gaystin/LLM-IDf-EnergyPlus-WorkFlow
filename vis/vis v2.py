...
# Keep city sequence consistent with carbon plots: Harbin -> Kunming.
CITY_ORDER_CANONICAL = ["Harbin", "Beijing", "Shanghai", "Wuhan", "Chengdu", "Guangzhou", "Kunming"]
CITY_ALIASES_TO_CANONICAL = {
    "Harbin": "Harbin",
    "哈尔滨": "Harbin",
    "Beijing": "Beijing",
    "北京": "Beijing",
...
def _city_order_key(city: str) -> Tuple[int, str]:
    canonical = CITY_ALIASES_TO_CANONICAL.get(str(city).strip(), str(city).strip())
    try:
        idx = CITY_ORDER_CANONICAL.index(canonical)
    except ValueError:
        idx = len(CITY_ORDER_CANONICAL)
    return idx, str(city)


def _city_colors_like_carbon(cities: List[str]) -> Dict[str, tuple]:
    # Match vis_carbon-50 color logic: linear gradient from blue to red on ordered cities.
    ordered = sorted(set(cities), key=_city_order_key)
    n = max(len(ordered), 2)
...
def plot_iterations_distribution(run_df: pd.DataFrame, out_file: Path) -> None:
    ...
    city_mean = plot_df.groupby("city", as_index=False)["best_iteration"].mean()
    city_mean = city_mean.sort_values("city", key=lambda s: s.map(lambda c: _city_order_key(c)[0]))
    
    fig, ax = plt.subplots(figsize=(12, 6))
    cities = city_mean["city"].tolist()

    # Match colors in figures/tokens/city_total_tokens_relationship.svg exactly.
    # Order: Harbin -> Beijing -> Shanghai -> Wuhan -> Chengdu -> Guangzhou -> Kunming
    reference_hex = [
        "#4C78A8",
        "#6679A3",
        "#807A9E",
    ...
    city_color_map = {
        city: reference_hex[min(idx, len(reference_hex) - 1)]
        for idx, city in enumerate(cities)
    }
    bar_colors = [city_color_map[c] for c in cities]

    x = np.arange(len(cities))
    y = city_mean["best_iteration"].to_numpy(dtype=float)
    bars = ax.bar(
        x,
        y,
        width=0.8,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(cities, rotation=0, ha="center", fontsize=11)
    ax.set_xlabel("City")
