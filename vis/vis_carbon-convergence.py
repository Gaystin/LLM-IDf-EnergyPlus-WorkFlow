...
DEFAULT_ROOT = Path("各城市迭代结果_早停")
DEFAULT_TREND_PNG = Path("city_carbon_iteration_trend_convergence.png")
DEFAULT_TREND_SVG = Path("city_carbon_iteration_trend_convergence.svg")
DEFAULT_FINAL_PNG = Path("city_carbon_best_vs_baseline_convergence.png")
DEFAULT_FINAL_SVG = Path("city_carbon_best_vs_baseline_convergence.svg")
DEFAULT_AVG_PNG = Path("city_carbon_avg_vs_baseline_convergence.png")
DEFAULT_AVG_SVG = Path("city_carbon_avg_vs_baseline_convergence.svg")
DEFAULT_ITER_CSV = Path("climate_carbon_iteration_summary_convergence.csv")
DEFAULT_FINAL_CSV = Path("climate_carbon_final_summary_convergence.csv")
...
# Distinct blue-to-red palette for the iteration trend chart only.
# Ordered to maximize visual separation between adjacent city lines.
TREND_DISTINCT_COLORS = [
    "#5582B1",  # deep blue
    "#6CA7C3",  # bright sky blue
    "#55ABA1",  # teal
    "#869E62",  # yellow-green
    "#EDC787",  # amber
    "#EDA069",  # orange
...
def _trend_city_colors(cities: List[str]) -> dict[str, tuple]:
    if len(cities) <= len(TREND_DISTINCT_COLORS):
        return {city: mcolors.to_rgba(TREND_DISTINCT_COLORS[idx]) for idx, city in enumerate(cities)}

    # Fallback for unexpected city counts while keeping a broad blue->red span.
    cmap = plt.get_cmap("turbo")
    vals = np.linspace(0.08, 0.92, len(cities))
    return {city: cmap(v) for city, v in zip(cities, vals)}
...
def load_city_round_metrics(path: Path) -> pd.DataFrame:
    ...
    if "run_summary" not in xls.sheet_names:
        raise ValueError(f"Workbook missing required sheet: run_summary | file={path}")

    if "run_round_metrics" in xls.sheet_names:
        detail_sheet = "run_round_metrics"
    elif "round_details" in xls.sheet_names:
        detail_sheet = "round_details"
    else:
        raise ValueError(
            f"Workbook missing required detail sheet: run_round_metrics or round_details | file={path}"
        )

    run_summary = pd.read_excel(path, sheet_name="run_summary")
    round_metrics = pd.read_excel(path, sheet_name=detail_sheet)
    ...
    if "iteration" in round_metrics.columns:
        # Early-stop folder uses 1-based iteration where first round is baseline simulation.
        # Shift to 0-based so iteration=0 is baseline, matching plotting semantics.
        round_metrics["iteration"] = pd.to_numeric(round_metrics["iteration"], errors="coerce") - 1
...
def plot_city_iteration_trend(iter_summary: pd.DataFrame, output_png: Path, output_svg: Path, show_plot: bool) -> None:
    ...
    color_map = _trend_city_colors(cities)
...
def plot_city_baseline_vs_target(
    ...
    # Place markers on the boundary between the two bars (right edge of the first bar).
    line_x = x
    ax_right.plot(
        line_x,
        saving_ratio_values,
