...
"""
        ...
        try:
            ...
            while category_count < min_categories and category_attempt < max_repair_attempts:
                category_attempt += 1
                self.logger.warning(
                    f"【类别覆盖校验】当前覆盖{category_count}/{min_categories}，需要纠偏"
                )
                self._log_repair_step("类别覆盖校验", category_attempt, "start")
...
"""
            ...
            if temp_issue:
                ...
                self._log_repair_step("温度映射校验", 1, "start")
                self._log_rejected_plan_reasoning(
                    "温度映射规则不满足，不可取建议",
                    plan,
                    rejected_reasoning_seen,
...
"""
            ...
            while not is_diverse and diversity_issue and diversity_attempt < max_repair_attempts:
                diversity_attempt += 1
                self.logger.warning(f"【字段多样性校验】未通过：{diversity_issue}，需要纠偏")
                self._log_repair_step("字段多样性校验", diversity_attempt, "start")
                self._log_rejected_plan_reasoning(
                    "字段多样性不满足，不可取建议",
                    plan,
                    rejected_reasoning_seen,
...
"""
    ...
    def _log_rejected_plan_reasoning(self, title, candidate_plan, dedup_cache=None, reason=None, attempt_label=None):
        """在告警场景打印不满足要求方案的推理文本，包含原因与统一格式。"""
        reasoning = ""
        ...
        if dedup_cache is not None:
            if fingerprint in dedup_cache:
                if attempt_label:
                    self.logger.info(f"【{attempt_label}】当前候选与上一条不可取候选完全相同，仅跳过重复打印；本次纠偏流程仍继续执行")
                return
        ...
        self._log_plan_reasoning(
            stage=f"不可取候选方案{f' - {attempt_label}' if attempt_label else ''}",
            plan=candidate_plan,
            level="warning"
        )

    def _log_repair_step(self, check_name, attempt, phase, result=None):
        """统一记录纠偏步骤，格式固定为“第N次纠偏启动/结束”。"""
        if phase == "start":
            self.logger.info(f"【{check_name}】第{attempt}次纠偏启动")
            return
    ...
    def _format_reasoning_numbered(self, reasoning_text):
        ...
        return "\n".join([f"{idx}. {item}" for idx, item in enumerate(parts, start=1)])
    ...
    def _log_plan_reasoning(self, stage, plan, level="info"):
        ...
        try:
            ...
            # 丰富度: 修改项总数（所有mod中fields总和）
            num_mods = sum(len(m.get('fields', {})) for m in mods if isinstance(m.get('fields', {}), dict))
            # 对象类别数
            num_categories = len({str(m.get('object_type', '')).upper().strip() for m in mods if m.get('object_type')})
            # 唯一字段数
            unique_fields = self._extract_plan_field_keys(plan)
            unique_fields_count = len(unique_fields)

            # 相似度：与上一轮字段的重复率（若无上一轮字段则为0）
            try:
                last_round = set([f.upper() for f in (getattr(self, 'last_round_fields', set()) or set())])
            except Exception:
                last_round = set()
            overlap_ratio = 0.0
            if unique_fields_count > 0 and last_round:
                overlap_ratio = len(unique_fields & last_round) / float(unique_fields_count)

            metrics_line = (
                f"【建议指标】丰富度: {num_mods}项修改 (唯一字段 {unique_fields_count}), "
                f"对象类别: {num_categories}个；相似度: 与上一轮字段重复率 {overlap_ratio:.1%}"
            )
    ...
    def _print_parallel_final_report(self):
        ...
        try:
            ...
            self.logger.info(f"{'─'*80}")
            self.logger.info(f"并行流程总耗时(墙钟): {float(getattr(self, 'parallel_total_duration_sec', 0.0) or 0.0):.2f}s")
