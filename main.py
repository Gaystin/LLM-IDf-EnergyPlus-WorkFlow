...
【强制多对象修改要求】
**本轮优化必须涉及至少5个不同类别的对象修改。**可覆盖以下类别（仅示例，不是固定优先顺序）：
- 照明  - 照明功率密度
- 设备 - 设备功率密度  
- 围护结构 - 导热系数
- 窗户 - 太阳热得系数
...
优先避免重复上轮字段及历史高频字段，尽量采用历史低频或未出现的对象字段组合。
...
"""
        ...
        try:
            ...
            # 首轮方案先做一次无token的低频补全，减少后续重试概率
            initial_augmented_plan = self._force_plan_diversity(
                plan,
                kb_context,
                target_min_categories=5,
            ...
            min_categories = 5
            max_repair_attempts = max(3, min(8, len(kb_context.get('candidates', [])) // 3 if isinstance(kb_context, dict) else 3))
            ...
            while category_count < min_categories and category_attempt < max_repair_attempts:
                category_attempt += 1
                self.logger.warning(
                    f"LLM方案仅覆盖{category_count}个优化方面，低于要求的{min_categories}个，触发纠偏重生成（第{category_attempt}/{max_repair_attempts}次）"
                )
                ...
                novelty_directive = self._build_novelty_directive(plan)
...
2) 必须优先使用“过去较少出现或未出现”的对象+字段组合，减少与上一轮重复
...
"""
            ...
            if temp_issue:
                ...
                self._log_rejected_plan_reasoning("温度映射规则不满足，不可取建议", plan, rejected_reasoning_seen)
                novelty_directive = self._build_novelty_directive(plan)
...
"""
                ...
                if temp_repaired_plan:
                    ...
                    if not repaired_issue:
                        ...
                        self._log_rejected_plan_reasoning("温度映射纠偏后仍不满足，不可取建议", temp_repaired_plan, rejected_reasoning_seen)
            ...
            while not is_diverse and diversity_issue and diversity_attempt < max_repair_attempts:
                diversity_attempt += 1
                self.logger.warning(f"字段多样性检查未通过：{diversity_issue}，触发纠偏重生成（第{diversity_attempt}/{max_repair_attempts}次）")
                self._log_rejected_plan_reasoning("字段多样性不满足，不可取建议", plan, rejected_reasoning_seen)
                novelty_directive = self._build_novelty_directive(plan)
...
"""
                ...
                if diversity_repaired_plan:
                    ...
                    if repaired_is_diverse:
                        ...
                        self._log_rejected_plan_reasoning("多样性纠偏后仍不满足，不可取建议", candidate_plan, rejected_reasoning_seen)
                        plan = candidate_plan
                        is_diverse = False
    ...
    def _force_plan_diversity(self, origin_plan, kb_context, target_min_categories=5, enforce_novelty=True):
        ...
        try:
            ...
            for _, target_type, field_name, field_key in supplement_pool:
                ...
                if enforce_novelty and field_key in last_round_fields and len(supplement_pool) > max_additions:
                    continue
            ...
            if added_count == 0:
                self.logger.warning("自动多样性重排未找到可补充的有效字段")
                return None

            self.logger.info(
                f"自动多样性重排：补充了{added_count}项字段修改；对象类别覆盖由{len(origin_categories)}提升到{len(covered_categories)}"
            )
            return forced_plan
    ...
    def _log_rejected_plan_reasoning(self, title, candidate_plan, dedup_cache=None):
        """在告警场景打印不满足要求方案的 reasoning（仅文本，不打印JSON）。"""
        reasoning = ""
        ...
        if reasoning:
            ...
            self.logger.warning(f"【{title}】未提供可用 reasoning 文本")
    ...
    def _build_novelty_directive(self, rejected_plan=None, max_items=10):
        ...
        if rejected_keys:
            lines.append("3) 以下为当前被判定不合格方案涉及字段，重生成时不得原样重复：")
            for key in rejected_keys[:max_items]:
                lines.append(f"   - {key}")

        lines.append("4) 请优先从候选中选择历史低频或未出现字段，并确保与上一轮至少50%不同。")
        lines.append("5) 若可行字段不足，请扩大对象类型覆盖，不要在同一对象字段上重复微调。")
        return "\n".join(lines)
