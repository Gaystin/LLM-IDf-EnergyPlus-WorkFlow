...
class EnergyPlusOptimizer:
    ...
    def __init__(
        ...
        try:
            ...
            self.client = True  # 标志已初始化
            self.logger.info("✓ Qwen客户端初始化成功")
        except Exception as e:
            self.logger.error(f"初始化Qwen失败: {e}")
            self.client = None
...
"""
    ...
    def _build_novelty_directive(self, rejected_plan=None, max_items=10):
        ...
        rejected_keys = self.knowledge_base.extract_plan_field_keys(rejected_plan) if rejected_plan else set()
        return self.knowledge_base.build_novelty_directive(
            last_round_fields=getattr(self, "last_round_fields", set()) or set(),
            field_modification_history=getattr(self, "field_modification_history", {}) or {},
            rejected_plan_keys=rejected_keys,
            max_items=max_items,
        )
    ...
    def _build_retry_anti_repeat_directive(self, rejected_plan=None, max_items=12):
        ...
        rejected_keys = self.knowledge_base.extract_plan_field_keys(rejected_plan) if rejected_plan else set()
        return self.knowledge_base.build_retry_anti_repeat_directive(
            last_round_fields=getattr(self, "last_round_fields", set()) or set(),
            rejected_plan_keys=rejected_keys,
            max_items=max_items,
        )
    ...
    def _build_balanced_expression_for_field(self, object_type, field_name):
        ...
        return self.knowledge_base.build_balanced_expression_for_field(object_type, field_name)
    ...
    def _request_plan_from_llm(self, system_prompt, user_prompt, temperature=0.0):
        ...
        self.logger.info(f"\n🤖 【调用LLM】第{call_index}次调用 - 模型: qwen-3.5-plus")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Qwen API错误: {response.message}")
    ...
    def _contains_threshold_intent(self, text):
        """判断文本是否包含阈值/上下限语义。"""
        return self.knowledge_base.contains_threshold_intent(text)
    ...
    def _get_field_usage_summary(self, max_high_freq=10, max_low_freq=10):
        ...
        return self.knowledge_base.get_field_usage_summary(
            field_modification_history=getattr(self, "field_modification_history", {}) or {},
    ...
    def _check_field_diversity(self, plan):
        """检查优化方案的字段多样性。"""
        return self.knowledge_base.check_field_diversity(
            plan=plan,
            last_round_fields=getattr(self, "last_round_fields", set()) or set(),
            field_modification_history=getattr(self, "field_modification_history", {}) or {},
        )
    ...
    def _get_temperature_mapping_issue(self, plan, user_request):
        ...
        return self.knowledge_base.get_temperature_mapping_issue(plan, user_request)
    ...
    def _is_special_value(self, value):
        ...
        return self.knowledge_base.is_special_value(value)

    def _is_numeric_value(self, value):
        """判断字段值是否为可计算的数值"""
        return self.knowledge_base.is_numeric_value(value)
    ...
    def _do_modification_work(self, plan, output_idf_path, source_idf_path):
        ...
        def _format_change_info(old_value, new_value, coefficient):
            """格式化修改幅度，兼容乘除与加减表达式。"""
            try:
                old_num = float(old_value)
                new_num = float(new_value)
            except (ValueError, TypeError):
                return " [无法计算变化量: 非数值字段]"
            ...
            if old_num != 0:
                ratio = delta / abs(old_num)
                if delta > 0:
                    return f" [增加 {ratio:.2%} ({delta:+.6g})]"
                return f" [降低 {abs(ratio):.2%} ({delta:+.6g})]"

            # old_num == 0 时，百分比无意义，使用绝对增减量展示
            if delta > 0:
                return f" [增加 {delta:+.6g} (基值为0，无法计算百分比)]"
            return f" [降低 {delta:+.6g} (基值为0，无法计算百分比)]"
        ...
        for obj_key, updates_list in sorted(updates_by_obj.items()):
            ...
            for update in updates_list:
                ...
                field = update['field']
                coef_info = _format_change_info(old_val, new_val, coef)
    ...
    def _generate_parallel_energy_plots(self, all_workflows):
        ...
        try:
            ...
            for workflow_id in sorted(all_workflows.keys()):
                ...
                workflow_plot_svg_path = os.path.join(plot_dir, f"{workflow_id}_cooling_heating_curve_{timestamp}.svg")
                fig.savefig(workflow_plot_path, bbox_inches='tight', dpi=150)
                fig.savefig(workflow_plot_svg_path, bbox_inches='tight')
                plt.close(fig)

                self.logger.info(
                    f"[{workflow_id}] 已保存单工作流能耗曲线: {workflow_plot_path} | {workflow_plot_svg_path}"
                )
                ...
                workflow_total_plot_svg_path = os.path.join(
                    plot_dir,
                    f"{workflow_id}_total_site_energy_curve_{timestamp}.svg"
                )
                fig_total.savefig(workflow_total_plot_path, bbox_inches='tight', dpi=150)
                fig_total.savefig(workflow_total_plot_svg_path, bbox_inches='tight')
                plt.close(fig_total)

                self.logger.info(
                    f"[{workflow_id}] 已保存单工作流总建筑能耗曲线: "
                    f"{workflow_total_plot_path} | {workflow_total_plot_svg_path}"
                )
            ...
            if valid_workflows:
                ...
                summary_plot_svg_path = os.path.join(plot_dir, f"all_workflows_cooling_heating_curve_{timestamp}.svg")
                fig.savefig(summary_plot_path, bbox_inches='tight', dpi=150)
                fig.savefig(summary_plot_svg_path, bbox_inches='tight')
                plt.close(fig)

                self.logger.info(f"已保存并行汇总能耗曲线: {summary_plot_path} | {summary_plot_svg_path}")
                ...
                total_summary_plot_svg_path = os.path.join(
                    plot_dir,
                    f"all_workflows_total_site_energy_curve_{timestamp}.svg"
                )
                fig_total_sum.savefig(total_summary_plot_path, bbox_inches='tight', dpi=150)
                fig_total_sum.savefig(total_summary_plot_svg_path, bbox_inches='tight')
                plt.close(fig_total_sum)

                self.logger.info(
                    f"已保存并行汇总总建筑能耗曲线: "
                    f"{total_summary_plot_path} | {total_summary_plot_svg_path}"
                )
            else:
    ...
    #     self.logger.info(f"{'█'*80}\n")
        ...
        #try:
            ...
            # 不同API key有不同的配额限制，可以从Qwen dashboard查看
            ...
            #self.logger.info(f"")
            #self.logger.info(f"⚠️  注意：剩余Token为估算值，实际配额请登录Qwen账户查看")
...
if __name__ == "__main__":
    ...
    IDF_PATH = "in.idf"
    IDD_PATH = "Energy+.idd"
    API_KEY_PATH = "api key/api_key_qwen3.5plus.txt"
    WEATHER_DIR = "weather"
