...
2. **相关参数建议协同调整（避免固定绑定）**
    - 新风的 Outdoor_Air_Flow_per_Person、Outdoor_Air_Flow_per_Zone_Floor_Area，必要时也可评估 Outdoor_Air_Flow_Air_Changes_per_Hour
    - 应尽量避免一增一减导致总新风量意外上升
...
10. **优化新风量（建议协同评估多个新风参数）**
  ...
  - 换气次数形式的新风量（Outdoor_Air_Flow_Air_Changes_per_Hour）：在满足健康标准前提下可谨慎优化
...
- 对 Lights / OtherEquipment / ElectricEquipment / ZoneInfiltration:DesignFlowRate：
  必须根据该对象的 Calculation Method 只选择当前生效字段（如 Watts/Area -> Watts_per_Zone_Floor_Area，旧版兼容 Watts_per_Floor_Area）。
...
- **平衡原则**：
  ...
  * **每轮优化应同时涉及多个类别的措施**（如同时优化围护结构+HVAC+内热源）
    * **新风参数组建议协同调整**：
        - Outdoor_Air_Flow_per_Person、Outdoor_Air_Flow_per_Zone_Floor_Area，可按场景灵活组合优化，必要时也可结合 Outdoor_Air_Flow_Air_Changes_per_Hour
  ...
  - 每句话后面必须加句号。
{
  ...
  "modifications": [
    {
      "object_type": "对象类型1（如Lights、OtherEquipment、ElectricEquipment、Material等）",
      "name_filter": null,
      "fields": {
        "字段名": "修改表达式"
      }
...
✅ 示例：modifications 中可包含 [Lights, OtherEquipment, ElectricEquipment, Material, ZoneInfiltration, ZoneHVAC:IdealLoadsAirSystem] 中的多个
...
{hvac_context if hvac_context else ""}
...
2. **主动识别所有可能的优化方向（不要遗漏）**
   ...
   - 如果知识库返回Daylighting:Controls → 考虑"照明调光与照度设定优化"
   - 如果知识库返回DesignSpecification:ZoneAirDistribution → 考虑"空气分配效率优化"
