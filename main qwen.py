...
class EnergyPlusOptimizer:
    ...
    def __init__(
        ...
        # 初始化Qwen客户端（DashScope OpenAI兼容接口）
        self.llm_model = "qwen3.5-plus"
        self.qwen_base_url = "https://dashscope.aliyuncs.com/compatible-mode/v1"
        try:
            with open(api_key_path, 'r') as f:
                api_key = f.read().strip()
            self.client = OpenAI(api_key=api_key, base_url=self.qwen_base_url)
            self.logger.info(f"✓ Qwen客户端初始化成功 (model={self.llm_model})")
        except Exception as e:
...
"""
    ...
    def _request_plan_from_llm(self, system_prompt, user_prompt, temperature=0.0):
        ...
        self.logger.info(f"\n🤖 【调用LLM】第{call_index}次调用 - 模型: {self.llm_model}")
    ...
    #     self.logger.info(f"{'█'*80}\n")
        ...
        # Qwen3.5-Plus 的常见配额范围，这里以一个示例值展示
        ...
        #try:
            ...
            # 不同API key有不同的配额限制，可以从DashScope控制台查看
            ...
            #self.logger.info(f"")
            #self.logger.info(f"⚠️  注意：剩余Token为估算值，实际配额请登录DashScope账户查看")
