        ...
        for mod in plan["modifications"]:
            ...
            for obj in self.base_idf.idfobjects[target_obj_type]:
                ...
                for field_input, value_expr in fields_to_mod.items():
                    ...
                    # 回退：当LLM给出语义字段名而非精确字段名时，按对象Calculation Method解析当前生效字段
                    if not target_attr:
                        method_field = self._get_method_target_field(target_obj_type, obj)
                        if method_field and method_field in valid_attrs:
                            target_attr = method_field
                            self.logger.info(
                                f"  [字段回退] {target_obj_type} ({obj_name}): "
                                f"'{clean_field}' 未精确匹配，按 Calculation Method 映射为 '{target_attr}'"
                            )
                    ...
                    # ========== 新增：验证字段是否与该对象的Calculation Method相匹配 ==========
                    if not self._is_field_for_method(target_obj_type, obj, target_attr):
                        method_field_map = {
                            "Lights": "Design_Level_Calculation_Method",
                            "ElectricEquipment": "Design_Level_Calculation_Method",
                            "ZoneInfiltration:DesignFlowRate": "Design_Flow_Rate_Calculation_Method",
                        }
                        current_method = getattr(obj, method_field_map.get(target_obj_type, ""), "Unknown")
                        correct_field = self._get_method_target_field(target_obj_type, obj) or "Unknown"
                        self.logger.warning(
                            f"  [警告：字段不匹配Method] {target_obj_type} ({obj_name}): "
                            f"要改'{target_attr}'但当前Method='{current_method}'对应字段应为'{correct_field}'，"
                            f"这个修改可能改错了！但仍将继续执行，请确认是否需要。"
                        )
                    # ========== END 新增 ==========
