...
class EnergyPlusOptimizer:
    ...
    def __init__(
        ...
        try:
            ...
            self.logger.error(f"初始化Gemini失败: {e}")
            self.client = None
...
"""
    ...
    def _request_plan_from_llm(self, system_prompt, user_prompt, temperature=0.0):
        ...
        self.logger.info(f"\n🤖 【调用LLM】第{call_index}次调用 - 模型: gemini-3.1-pro")
...
{user_prompt}"""
    ...
    #     self.logger.info(f"{'█'*80}\n")
        ...
        # Gemini 2.0 Flash的常见配额范围，这里以一个示例值展示
        ...
        #try:
            ...
            # 不同API key有不同的配额限制，可以仯Google Cloud Console查看
            ...
            #self.logger.info(f"")
            #self.logger.info(f"⚠️  注意：剩余Token为估算值，实际配额请登录Google Cloud账户查看")
...
if __name__ == "__main__":
    ...
    IDF_PATH = "in.idf"
    IDD_PATH = "Energy+.idd"
    API_KEY_PATH = "api key/api_key_gemini.txt"
    WEATHER_DIR = "weather"
